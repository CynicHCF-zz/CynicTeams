...
public class KitListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        Kit kit = HCF.getInstance().getMapHandler().getKitManager().get(player.getUniqueId(), sign.getLine(1));

        if (kit.getName().equalsIgnoreCase("foxtrot")){
    ...
    public static void attemptApplyKit(Player player, Kit kit) {
        ...
        if (AquaCoreAPI.INSTANCE.getPlayerData(player.getUniqueId()).isInStaffMode()) {
    ...
    /*    if (!HCF.getInstance().getMapHandler().getKitManager().canUseKit(player, kit.getName())) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&eYou do not own this kit. Purchase it at " + HCF.getInstance().getConfig().getString("store")));
            return;
        }*/
    ...
    public void helo(PlayerMoveEvent event) {
        ...
        if (location.getBlock().getType() == Material.STONE_PLATE) {
            if (location.getBlock().getRelative(BlockFace.DOWN).getType() == Material.OBSIDIAN) {
                player.setVelocity(player.getLocation().getDirection().multiply(7.5).setY(1.5));
                player.setMetadata("noflag", new FixedMetadataValue(HCF.getInstance(), true));
                new BukkitRunnable() {

                    public void run() {
                        player.removeMetadata("noflag", HCF.getInstance());
                    }
                }.runTaskLater(HCF.getInstance(), 100);
        ...
        if (location.getBlock().getType() == Material.GOLD_PLATE) {
            if (location.getBlock().getRelative(BlockFace.DOWN).getType() == Material.OBSIDIAN) {
                ...
                }.runTaskLaterAsynchronously(HCF.getInstance(), 100);
                noDamage.put(player.getName(), true);
            }
        }
