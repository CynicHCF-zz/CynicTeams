package com.cynichcf.hcf.events.region.glowmtn;

import com.cynichcf.hcf.events.region.glowmtn.listeners.GlowListener;
...
public class GlowHandler {
    ...
    public GlowHandler() {
        try {
            ...
            if (!file.exists()) {
                ...
                glowMountain = Library.GSON.fromJson(FileUtils.readFileToString(file), GlowMountain.class);
        ...
        HCF.getInstance().getServer().getScheduler().runTaskTimer(HCF.getInstance(), () -> {
            ...
            // Broadcast the reset
            Bukkit.broadcastMessage(ChatColor.GOLD + "[Glowstone Mountain]" + ChatColor.LIGHT_PURPLE + " All glowstone has been reset!");
    ...
    public void save() {
        try {
            FileUtils.write(file, Library.GSON.toJson(glowMountain));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
