...
public class KitMapListener implements Listener {
    ...
    private int getAdditional(Player killer) {
        if (killer.hasPermission("foxtrot.killreward.hades")) {
            ...
            return 50;
        } else if (killer.hasPermission("foxtrot.killreward.foxtrot")) {
            return 75;
    ...
    public void onProjectileHit(ProjectileHitEvent event) {
        Bukkit.getScheduler().runTaskLaterAsynchronously(HCF.getInstance(), event.getEntity()::remove, 1L);
    }
    ...
    public void onQuit(PlayerQuitEvent event) {
        HCF.getInstance().getMapHandler().getKitManager().logout(event.getPlayer().getUniqueId());
    }

    @EventHandler
    public void onJoin(PlayerJoinEvent event) {
        HCF.getInstance().getMapHandler().getKitManager().loadKits(event.getPlayer().getUniqueId());
    }
