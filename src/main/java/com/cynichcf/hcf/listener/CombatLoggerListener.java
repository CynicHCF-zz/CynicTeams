...
import rip.lazze.libraries.serializable.PlayerInventorySerializer;
...
public class CombatLoggerListener implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity().hasMetadata(COMBAT_LOGGER_METADATA)) {
            ...
            int victimKills = HCF.getInstance().getKillsMap().getKills(event.getEntity().getUniqueId());
            ...
            if (event.getEntity().getKiller() != null) {
                ...
                int killerKills = HCF.getInstance().getKillsMap().getKills(event.getEntity().getKiller().getUniqueId());
            ...
            if (villager.getWorld().getEnvironment() == World.Environment.THE_END) {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        // store the death amount -- we'll use this later on.
                        int victimKills = HCF.getInstance().getKillsMap().getKills(metadata.playerUUID);
